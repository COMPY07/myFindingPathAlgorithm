import pygame as p
from .BoardType import Type as bt
class Grid(bt):
    def __init__(self, screen : p.Surface = None, row : int = 5,
                 col : int = 5, space : float = -1.5):
    ...
    def changeBox(self, *inp):#x: int, y: int, color=0):
        box: Box
        if len(inp) > 2:
            x, y = inp[0], inp[1]
            box = self.board[y][x]
            if type(inp[2]) == int: color : int = inp[2]
            else: color : str = inp[2]
        else:
            box = inp[0]
            if type(inp[1]) == int: color : int = inp[1]
            else: color : str = inp[1]
        # print(inp, box, color)
        if type(color) == int:
...
class Box:
    ...
    def draw(self, screen : p.Surface):
        p.draw.rect(screen, self.color, [self.x, self.y, self.width, self.height]) # x, y, width, height
    def __str__(self):
